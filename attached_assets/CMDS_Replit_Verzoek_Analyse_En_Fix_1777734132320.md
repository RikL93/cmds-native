# CMDS — Verzoek tot analyse & fix-implementatie native shell

> **Voor**: Replit native developer team
> **Van**: CMDS productteam
> **Datum**: 2 mei 2026
> **Bijlage**: `Screenshot_20260502_165039_CMDS.jpg` (Ingest Diagnose log van reproductiemoment)
> **Prioriteit**: Hoog — blokkeert betrouwbare GPS-tracking in productie

---

## 1. Aanleiding

Op een echt Android-toestel met BRAVO 1 actief gekoppeld is zojuist het **exacte failure-patroon** gereproduceerd waar we al langer mee worstelen. Telefoon werd vergrendeld, de tracking-status sloeg op **oranje** (= unit lijkt offline). De bijgevoegde screenshot van de Ingest Diagnose log toont de root cause helder.

Dit document bevat:
- **Deel A** — De diagnose en het verzoek (deze sectie + sectie 2-4)
- **Deel B** — De volledige technische implementatiegids (sectie 5 en verder, voorheen `CMDS_Replit_Native_Fix_Prompt.md` v2)

Eén bestand om te delen, geen losse fragmenten meer.

---

## 2. Diagnose op basis van bijgevoegde screenshot

Chronologie uit de log (onderaan = oudste, bovenaan = nieuwste):

1. **`14:50:25.099–103`** — Burst van ~25 `POST_ATTEMPT` events richting `ingest-location`, **zonder enige `POST_RESPONSE`**. De native shell probeerde backlogged coördinaten te versturen, maar requests hingen of werden gedropt.
2. **`14:50:25.175`** — `TOKEN_REFRESH — onSupabaseTokenRefreshed`. Het Supabase access-token was verlopen en werd reactief ververst.
3. **`14:50:25.190`** — **`SERVICE_STATE running=false`**. De tracking-service stond op dat moment stil.
4. **`14:50:25.625–655`** — Service herstart, `onUnitLinked uid=ddb85933 src=BRAVO 1`, en pas dán beginnen de `POST_RESPONSE HTTP 200` events weer binnen te komen.

Edge function logs aan onze backend-kant bevestigen het patroon: in `whoami-unit` zien we expliciet **`Error: JWT has expired`** rond datzelfde moment.

---

## 3. Root cause (drievoudig)

1. **WebView wordt gepauzeerd door Doze-mode** bij schermlock → `SERVICE_STATE=false` → coords worden wel verzameld maar niet betrouwbaar gePOST.
2. **Token-refresh is reactief, niet proactief** → het token verloopt "in flight" → een hele burst POSTs faalt stilletjes voordat de refresh rondkomt.
3. **Geen retry-queue voor failed POSTs** → die ~25 attempts gaan verloren ipv opnieuw verstuurd te worden mét vers token → gat van seconden tot minuten → backend markeert unit als stale → **oranje bol** in de UI.

---

## 4. Verzoek aan Replit

1. **Analyseer de bijgevoegde screenshot** (`Ingest Diagnose` log) en bevestig dat jullie hetzelfde patroon zien dat hierboven beschreven staat.
2. **Implementeer de fixes uit Deel B (sectie 5 en verder)** — dit is dezelfde inhoud als het eerder gedeelde `CMDS_Replit_Native_Fix_Prompt.md`, hier ter referentie volledig opnieuw bijgevoegd zodat alles in één bestand zit. Specifiek raken deze secties direct het hierboven beschreven patroon:
   - **Sectie 9 (LocationTrackingService)** — true Foreground Service met PARTIAL_WAKE_LOCK + persistent notification — voorkomt `running=false` bij schermlock
   - **Sectie 10 (LocationPoster)** — queue + exponential backoff retry op 401 — zorgt dat die ~25 hangende POSTs niet verloren gaan
   - **Sectie 11 (SupabaseTokenManager)** — **proactieve** refresh elke 25 min — token verloopt nooit meer "in flight"
   - **Sectie 12 (Watchdog)** — WorkManager periodic 5 min — herstart service als die toch sterft
   - **Sectie 14 (Onboarding)** — verplichte `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` flow — kritiek tegen Doze-mode op fabrikant-skins (Samsung/Xiaomi/Oppo)
3. **Houd de bestaande bridge-events overeind** (`onUnitLinked`, `onSupabaseTokenRefreshed`, `onLocationPosted`, `onServiceStateChanged`, etc.). Onze ingest-diagnose UI hangt eraan vast en is essentieel voor verdere validatie van de fix.

### Acceptance criteria (samengevat — uitgebreid in sectie 16)

De fix is pas opgeleverd als op een echt toestel:

- [ ] Eenheid gekoppeld → telefoon vergrendeld → na **30 min** nog groen
- [ ] Idem na **60 min** (cruciaal: dekt het token-refresh moment)
- [ ] Idem na **2 uur**
- [ ] Geen gap > 30s tijdens token-refresh
- [ ] Service herstart automatisch binnen 5 min na force-stop
- [ ] Queue houdt 2 min flight-mode vast en flusht alle items bij reconnect

Geef gerust een seintje als iets in onderstaande technische gids onduidelijk is, of als jullie eerst willen sparren voordat je begint te coden. Dank!

---
---

# DEEL B — Volledige technische implementatiegids

> Onderstaande inhoud is identiek aan het eerder gedeelde `CMDS_Replit_Native_Fix_Prompt.md` (v2) en consolideert + vervangt de oudere `CMDS_Native_Background_GPS_Implementation.md` (v1). Hier opnieuw bijgevoegd zodat dit ene bestand alles bevat.
