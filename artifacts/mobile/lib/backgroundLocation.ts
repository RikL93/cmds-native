...
/**
 * Vraagt de native diagnose-snapshot op (Doze, battery-opt, standby bucket,
 * manufacturer, model, sdkInt) en zet hem om in de partial detail die we
 * meegeven aan logIngest("service_state", ...). Op iOS / web geeft de
 * native module een lege snapshot en krijgen we een lege spread terug.
 *
 * Wordt ook hergebruikt in app/index.tsx voor de foreground_check_dead_*
 * service_state logs zodat élk service_state event diagnostiek bevat.
 */
export async function fetchDiagnosticsForLog(): Promise<{
  dozeMode?: boolean;
  ...
  try {
    const snap = await CmdsLocation.getDiagnosticsSnapshot();
    return {
      dozeMode: snap.dozeMode,
      batteryOptIgnored: snap.batteryOptIgnored,
      standbyBucket: snap.standbyBucket,
      standbyBucketLabel: snap.standbyBucketLabel,
