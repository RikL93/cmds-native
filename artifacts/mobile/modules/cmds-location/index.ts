...
type NativeMod = {
  ...
  openOemBatterySettings(): Promise<string | null>;
...
export const CmdsLocation = {
  ...
  getDiagnosticsSnapshot: (): Promise<DiagnosticsSnapshot> =>
    _mod?.getDiagnosticsSnapshot() ?? Promise.resolve({}),

  isBatteryOptimizationIgnored: (): Promise<boolean> =>
    _mod?.isBatteryOptimizationIgnored() ?? Promise.resolve(true),

  openOemBatterySettings: (): Promise<string | null> =>
    _mod?.openOemBatterySettings() ?? Promise.resolve(null),
